...
def my_zscore(X, mX=None, sX=None):
    ...
    sX_safe = np.where(sX == 0, 1, sX)


    zX = (X - mX) / sX_safe

    return mX, sX_safe, zX
...
def resp_stim_loader(config, subjectID, movIDs, featName=None, stim_delays=[2,3,4,5,6], loadResp=True, loadStim=True, show_progress=True):
...
def get_sig_voxel_inds(ps, alpha=0.05): # Get sig.voxel inds (FDR correction p<0.05)
    ...
    rejected, ps_correction = multitest.fdrcorrection(ps[nonan_mask], alpha=alpha, method='indep', is_sorted=False)
...
def get_test_results_corr(model, t_Stim_test, Resp_test, dims=[], ps_alpha = 0.05, intercept=True):
    ...
    if intercept == True:
        p_Resp_test = np.dot(t_Stim_test[:,dims], model.coef_[dims, :]) + model.intercept_.cpu().numpy()
    else:
